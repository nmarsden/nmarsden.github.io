angular.module('meg.home', ['ui.router']).config([
  '$stateProvider',
  ...
  '$modal',
  '$location',
  function HomeController($scope, $modal, $location) {
    var puz = [];
    puz[0] = [
      0,
      0,
    ...
    var initSquares = function (level) {
      ...
      for (; i < len; i = i + 5, row++) {
        squares.push({
          id: i,
          selected: !(levelData[row] & 1)
        });
        squares.push({
          id: i + 1,
          selected: !(levelData[row] & 2)
        });
        squares.push({
          id: i + 2,
          selected: !(levelData[row] & 4)
        });
        squares.push({
          id: i + 3,
          selected: !(levelData[row] & 8)
        });
        squares.push({
          id: i + 4,
          selected: !(levelData[row] & 16)
        });
      }
    };
    ...
    $scope.clickSquare = function (index) {
      var toggleIndexes = calcSquaresToToggle(index), i = 0, len = toggleIndexes.length;
      for (; i < len; i++) {
